...
class TileImprovementFunctions(val tile: Tile) {
    ...
    /**
     * Per-turn maintenance estimate of [improvement] on this tile, matching the logic in
     * [getTransportationUpkeep][com.unciv.logic.civilization.transients.CivInfoStatsForNextTurn.getTransportationUpkeep].
     * Read-only and shared by [ImprovementPickerScreen][com.unciv.ui.screens.pickerscreens.ImprovementPickerScreen]
     * and the Godot gateway; actual charging still happens in turn settlement, which may round differently.
     */
    @Readonly
    fun getMaintenance(improvement: TileImprovement, civ: Civilization): Stats {
        val maintenance = Stats()
        if (civ.getMatchingUniques(UniqueType.NoImprovementMaintenanceInSpecificTiles)
                .any { tile.matchesFilter(it.params[0], civ) }
        ) return maintenance

        val context = GameContext(civ, tile = tile)
        val maintenanceUniques = improvement.getMatchingUniques(UniqueType.ImprovementAllMaintenance, context) +
            ...
            (if (tile.getOwner() == civ) improvement.getMatchingUniques(UniqueType.ImprovementMaintenance, context) else emptySequence())
        ...
        civ.forEachMatchingUnique(UniqueType.RoadMaintenance) { unique ->
