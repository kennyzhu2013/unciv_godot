...
class ImprovementPickerScreen(
    ...
    fun accept(improvement: TileImprovement?, secondImprovement: TileImprovement? = null) {
        // 状态操作与 Godot 网关共用无 GUI 入口；此处只额外负责关闭屏幕。
        if (!WorkerImprovementActions.accept(tile, unit, improvement, secondImprovement, onAccept)) return
        game.popScreen()
    }
    ...
    private fun getMaintenance(improvement: TileImprovement): Stats =
        tile.improvementFunctions.getMaintenance(improvement, currentPlayerCiv)
