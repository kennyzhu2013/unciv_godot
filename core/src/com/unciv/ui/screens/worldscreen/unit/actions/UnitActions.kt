...
object UnitActions {
    ...
    /** 基础动作的无 GUI 入口；不枚举护送、升级等依赖 WorldScreen 的动作。 */
    fun getBasicActions(unit: MapUnit): Sequence<UnitAction> = sequence {
        addSkipAction(unit)
        addSleepActions(unit, unit.getTile())
        addFortifyActions(unit)
        yieldAll(UnitActionsFromUniques.getSetupActions(unit, unit.getTile()))
    }
    ...
    private suspend fun SequenceScope<UnitAction>.addSkipAction(unit: MapUnit, onSkipped: (() -> Unit)? = null) {
        yield(UnitAction(
